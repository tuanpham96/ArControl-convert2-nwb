...
This is forked from `ndx_beadl.plot` function by Tuan Pham @ March 1, 2023. 
"""
import pandas as pd
...
def show_by_type_and_value(table: Union[ActionsTable, EventsTable, pd.DataFrame],
        ...
        df_type = table_types.to_dataframe(index=True)
        ...
        if show_table_values:
            ...
            if value_field not in df:
                df = df.join(df_type, on = type_column)
            assert value_field in df, f"'{value_field}' cannot be found in the merged table of data and types"
            table_and_values_merged = list(zip(
                [table_types_loaded[i] for i in df[type_column][:]], 
                df[value_field][:]
            ))
            ...
            y_tick_labels = [
                f"{v[0]}" if v[1] is None else f"{v[0]} ({v[1]})"
                for v in table_types_and_values]
        # Show events by type
        else:
...
def plot_events(events: Union[EventsTable, pd.DataFrame],
    ...
    :param value_field: The field that indicates where to obtain value from. (Default="value")
    ...
    y_values, y_tick_labels, y_label = show_by_type_and_value(
        table=events, table_types=event_types, value_field=value_field
    )
...
def plot_actions(actions: Union[ActionsTable, pd.DataFrame],
    ...
    # show events by type and value
    y_values, y_tick_labels, y_label = show_by_type_and_value(
        table=actions, table_types=action_types, value_field=value_field
    )
    if fig is None:
        fig = plt.figure(figsize=(18, 3) if figsize is None else figsize)
...
def plot_states(states: Union[StatesTable, pd.DataFrame],
    ...
    :param description_field: The field that indicates where to obtain description from. 
                        If `None`, will not show anything. (Default=None)
    :param figsize: Figure size tuple
    :param fontsize: Fontsize (Default=18)
    ...
    if description_field is not None:
        y_tick_labels = [
            _name if _desc in ["", None] 
            else f'{_name} ({_desc})' 
            for _name, _desc in zip(
                state_types['state_name'][:], 
                state_types[description_field][:]
            )
        ]
    y_label = "State type"
...
def plot_trials(trials: Union[TrialsTable, pd.DataFrame],
                ...
                events: EventsTable,
                event_types: EventTypesTable,
                state_description_field: Optional[str] = None,
                action_value_field: str = 'value',
                event_value_field: str = 'value',
                figsize=None,
                fontsize=18,
                rectangle_height=1,
                marker_size=None):
    ...
    :param event_types: The EventTypesTable that corresponds to EventsTable
    :param state_description_field: The field that indicates where to obtain description from StatesTable. 
                        If `None`, will not show anything. (Default=None)
    :param action_value_field: The field that indicates where to obtain value from ActionsTable. (Default="value")
    :param event_value_field: The field that indicates where to obtain value from EventsTable. (Default="value")
    :param show_event_values: Separate events both by type and value (True) or by type only (False). (Default=True)
    :param marker: String marker to use in scatter plot. (Default="|")
    ...
    if len(actions_index) > 0:
        plot_actions(actions=actions[actions_index],
                     ...
                     value_field=action_value_field,
                     marker_size=marker_size,
                     marker_width=2,
                     marker_color='green',
                     y_offset=y_offset,
    ...
    if len(states_index) > 0 :
        plot_states(states=states[states_index],
                    ...
                    description_field = state_description_field,
                    y_offset=y_offset,
                    rectangle_height=1,
